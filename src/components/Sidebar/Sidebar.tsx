...
function CountBadge({ count, className }: CountBadgeProps) {
  return (
    <span
      aria-hidden
      className={`pointer-events-none absolute right-3 top-1/2 z-10 -translate-y-1/2 text-xs font-medium tabular-nums text-gray-500 transition-opacity ${className ?? ''}`.trim()}
    >
      {count}
    </span>
  );
