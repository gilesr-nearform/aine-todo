...
| Day 1 | Epic 12 added — theme switcher (light + dark) using gov.ie's official `[data-theme="govie-light"]` and `[data-theme="govie-dark"]` token sheets | User asked for a "theme switcher for light and dark mode" in the top right of the green header. Gov.ie already ships paired light/dark CSS variable sets in `@ogcio/theme-govie`, so switching at the document root flips every design-system component in place without overriding any token values — satisfying the project rule "Never override gov.ie token values." Globals import both `light.css` and `dark.css`; a small `ThemeContext` writes `data-theme` and `color-scheme` on `<html>` and persists the preference in localStorage (`listlens:v1:theme`). First-paint default follows `prefers-color-scheme`. Tailwind's `darkMode` is configured to follow `[data-theme="govie-dark"]` so we can layer `dark:` variants on the surfaces we authored (AppShell, Sidebar, input bar). The toggle uses gov.ie's `HeaderPrimaryMenu` + `HeaderMenuItemButton`, which gives us the built-in tool-item hover/focus styling. Sun and moon glyphs are inline SVGs because the gov.ie icon set doesn't ship them; this is the smallest possible custom-icon footprint and isn't an override of any design-system token. See `docs/stories/12-theme-switcher.md` for the explicit cuts (no system/auto mode UI, no per-component dark overrides on our side, no animation). |
