...
- **Delete** a todo (with confirm modal, Epic 03 — undo flow removed Day 1, see §9)
...
- Built using `@ogcio/design-system-react` components wherever the design system covers our needs; custom components only where it doesn't (e.g. swipe-to-delete, custom `ConfirmModal` wrapper, `TodoEditor` row)
- Voice and tone follow gov.ie plain-English guidance

### Out of scope for v1

> Two items from the official PRD's out-of-scope list were brought in-scope as Day-1 expansions and are recorded in §9: **filtering / sorting** (reorder via Epic 06; search via Epic 07) and **multi-list management** (Epic 08). The remainder still holds.

Per the official PRD:
- User accounts / authentication
- Task prioritisation
- Deadlines or due dates
- ~~Filtering or sorting~~ — *brought in-scope Day 1: reorder (Epic 06) + search (Epic 07). See §9.*
- Notifications
- Categorisation / tagging
- ~~Multi-list management~~ — *brought in-scope Day 1: Epic 08. See §9.*
...
| Gov.ie design system doesn't have a component we need (e.g. swipe-to-delete on mobile, a confirm modal wrapper, a row-level editor) | Medium | Defer to gov.ie components first; build custom components in the gov.ie *style* (using gov.ie tokens) only where the library lacks coverage. Document each custom component in `component-inventory.md`. |
...
| Day 1 | Material Symbols Outlined font self-hosted via the `material-symbols` npm package, and the sidebar's "All tasks" entry switched from `apps` to `list` | The gov.ie `Icon` component renders most of its curated icon names (including `apps`, `add_circle`, `more_horiz`, `edit`, `delete`, `menu`) via a `<span class="material-symbols-outlined">` ligature span — but neither `@ogcio/theme-govie` nor `@ogcio/design-system-react` ships the actual Material Symbols Outlined font. The result, before this fix, was that the design system's own icon API rendered the icon *name* as plain text in any browser without the font installed locally. First attempt loaded the font from Google's CDN with `preconnect` + `display=block`, but the runtime environment (Cursor's in-IDE browser, and likely any sandboxed preview) blocks external font requests, so zero glyphs ever rendered. Switched to the `material-symbols` npm package — the canonical community-maintained package, just CSS + woff2 — and imported `material-symbols/outlined.css` from `src/styles/globals.css`. This matches the project's existing self-hosted-font pattern: Lato (gov.ie's body font) ships transitively via `@ogcio/theme-govie`, which bundles `@fontsource/lato` and serves the woff2 from the bundle for the same reason — gov.ie's stylesheet expects the font to resolve from the bundle, not from a CDN. The font asset is ~340kB and ships alongside the bundle. Side effect: the runtime now accepts any valid Material Symbols name (the design system's `IconId` TypeScript type is narrower than the runtime accepts), which the user asked for to give the "All tasks" sidebar entry a list glyph that visually echoes the Apple Reminders icon. The cast to `IconId` is narrowed to one named constant `ALL_TASKS_ICON` in `Sidebar.tsx` so the escape hatch is auditable. New dependency `material-symbols` recorded here per the "No new dependencies without checking docs/architecture.md" rule — justified because it's required to make the existing gov.ie Icon API work at all (without it the design system's own curated icons render as their literal text names). |
| Day 1 | Undo flow removed entirely. Per-task delete is now guarded by a `ConfirmModal` (matching Clear completed and Delete list) instead. | Epic 03 shipped an `UndoToast` + `recentlyDeleted` queue + 4-second restore window as the safety net for the per-task delete action. After the Day-1 addition of `ConfirmModal` for Clear completed and Delete list, the per-task delete also picked up a modal — which left the app with two layered safety nets for a single action (confirm → delete → undo for 4s). The double-confirmation felt heavy and visually noisy (a toast permanently in the lower-third of the layout, competing with the task input bar). Decision: drop the undo half. The modal is now the single guard. Removed: `UndoToast` + `UndoToastContainer` components, the `useUndoTimer` hook, the `recentlyDeleted` slice from `AppState`, `UNDO_DELETE` and `EXPIRE_DELETED` actions, the `DeletedRecord` type, the `undo.deleted` / `undo.undo` translation keys, and the toast-render slot in `AppShell.tsx`. The per-task confirm body copy was updated from "You can undo this immediately afterwards." to "This can't be undone." to match the Clear-completed and Delete-list tone now that it's true. Story `docs/stories/03-delete-and-undo.md` retains its original acceptance criteria as historical record but is marked superseded in the stories index — implementation now matches the simpler delete-with-confirm pattern. Trade-off acknowledged: a confirm-then-mis-confirm is now unrecoverable from within the app; users who delete and immediately regret have to re-create the task. This matches every other destructive action in the app and is the conventional gov.ie-tone approach. |
| Day 1 | Theme toggle relocated from the header (right-side `HeaderPrimaryMenu`) to the bottom of the sidebar, under the Add-list form. Burger menu (mobile only) takes the freed right-side slot in the green header. | The header was carrying three control affordances on mobile (language toggle in the utility row, burger on the left of the logo, theme toggle on the right). The builder asked for a quieter header: the burger and theme are conceptually navigation/preferences, so collocating them in the sidebar reduces header noise. The sidebar already houses lists and the Add-list form; pinning the theme toggle below it gives a single home for preferences. Implementation notes: (a) the sidebar is now a two-region flex column — scrollable list rail at the top, non-flex footer carrying Add-list + theme toggle, so the toggle stays anchored to the bottom regardless of list count; (b) the burger is now a `HeaderMenuItemButton` inside `HeaderPrimaryMenu`, replacing the standalone `IconButton` — this gives it the gov.ie-native white-on-green colour cascade for free and removes the `appearance="dark"` workaround; (c) the `MobileNav` wrapper component is deleted (its drawer state lives in `Header` now, since gov.ie's `HeaderPrimaryMenu` filters children strictly and won't accept a fragment containing both the menu button and the drawer). |  User asked for a theme switcher in the top right of the green header. Gov.ie publishes `[data-theme="govie-light"]` and `[data-theme="govie-dark"]` selector sheets, so the application-level switch is officially supported via a single attribute on `<html>`. **However**, as of build, `@ogcio/theme-govie/dark.css` is byte-identical to `light.css` aside from the selector — gov.ie hasn't published real dark token values yet. To deliver an actually dark appearance we therefore override gov.ie's neutral primitives and gray scale ourselves under `[data-theme="govie-dark"]` in `src/styles/globals.css` (inverted scale: `gray-50` becomes near-black, `gray-950` becomes near-white, `neutral-white` becomes near-black). This is a deliberate, scoped exception to the project rule "never override gov.ie token values" (`CLAUDE.md`, `.cursor/rules/project.mdc`). The exception is constrained to neutral/gray primitives only — brand colours and intent tokens (success, error, warning, info, focus) are untouched, so the gov.ie identity remains intact and gov.ie components flip via their own semantic tokens that already cascade through those primitives. Tailwind utilities like `bg-gray-200` also resolve to the same CSS variables, so they flip together; only `bg-white` needed an explicit utility-level override because Tailwind hardcodes white as a literal. A small `ThemeContext` writes `data-theme` and `color-scheme` on `<html>` and persists the preference in localStorage (`listlens:v1:theme`). First-paint default follows `prefers-color-scheme`. The toggle itself uses gov.ie's `HeaderPrimaryMenu` + `HeaderMenuItemButton` for the built-in tool-item hover/focus styling; sun/moon glyphs are inline SVGs because gov.ie's IconId set doesn't ship them (the smallest possible custom-icon footprint). See `docs/stories/12-theme-switcher.md` for the cuts (no system/auto tri-state, no animation, no tuned per-component dark palette). When gov.ie publishes real dark tokens upstream, this override can be removed in one diff and the project rule restored without changing any consumer code. |

| Day 1 | Dark-mode token override narrowed: drop `--gieds-color-neutral-white` and `--gieds-color-base-white` global remappings; flip `--gieds-color-surface-system-neutral-default` directly instead | The original Day-1 dark-mode entry above remapped `neutral-white → emerald-900` to flip the page surface, modal popup, and mobile drawer. That same primitive is also the source for every "white text on dark fill" semantic token in gov.ie — `text-tone-primary-fill-default`, `text-tone-dark-fill-default`, `text-tone-light-*` (every variant), `icon-tone-primary-fill-default`, `icon-tone-dark-fill-default`, `icon-tone-light-*`, and `surface-tone-light-fill-default`. Remapping the primitive globally therefore inverted both the surfaces *and* every text/icon use that legitimately needs to stay white. The most consequential failure: the primary destructive button (used by all three `ConfirmModal` actions — `Delete '<task>'`, `Clear completed`, `Delete '<list>'`) rendered with dark-emerald text on a dark-emerald fill, a WCAG contrast failure on the most safety-critical UI in the app. Fix: stop overriding `neutral-white` and `base-white`, and override `--gieds-color-surface-system-neutral-default` directly (the only token actually needed for surface flipping — confirmed by checking gov.ie's `light.css`: no other token resolves to `surface-system-neutral-default` or `base-white`, so no cascade is at risk). The `.bg-white` Tailwind utility override at the bottom of `globals.css` was retargeted from `neutral-white` to `surface-system-neutral-default` to match. Net effect: the scoped exception to "never override gov.ie tokens" is now strictly narrower — neutral/gray scale (gray-50 through gray-950, plus neutral-50 for the SideNav selected state) and one surface token only. Brand and intent tokens remain untouched. Confirmed via in-IDE browser MCP: the Delete-task, Delete-list, and Clear-completed primary buttons now render white-on-emerald-800 (≈14:1 contrast) in dark mode; light mode is unchanged. |
